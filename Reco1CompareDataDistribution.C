...
void setStyleRatio(TH1D* h, TString file1_label, TString file2_label, TString histoXAxis){
    h->SetTitle("");
    h->SetNdivisions(510, "Y");
    h->GetYaxis()->SetTitle("#frac{sim-sample}{sample}"/*"#frac{("+file2_label+")-("+file1_label+")}{("+file1_label+")}"*/);
    h->GetYaxis()->SetTitleOffset(0.57);
    h->GetYaxis()->SetTitleSize(0.07);
    h->GetYaxis()->SetLabelSize(0.075);
    h->GetXaxis()->SetTitle(histoXAxis);
...
void CompareDataDistributions(TString gCurVersion, TString gRefVersion){
...
//   TString ref_xrootd_path=gSystem->Getenv("ref_dunetpc_ana_hist_xrootd_path");
  ...
  LoopHistos(folder_size, list_folder, list_histo, gCurVersion.Data());
  ...
  TFile *RefFile = TFile::Open( gRefVersion.Data());
  TFile *CurFile = TFile::Open( gCurVersion.Data());

  // TFile *histFile = new TFile("reco1_histos_comparison.root", "RECREATE");
  ...
  TString plotname[] = {
    ...
    "Beam angle Z",
    "Theta",
    "Zenit angle weighted",
    "Azimut angle weighted",
    "Deposit energy",
    "Num photons",
    "Number of electrons",
    "dcosy weighted",
    "Deposit energy-CC-nue",
    "Deposit energy-CC-numu",
    "Deposit energy-NC",
    "Num photns-CC-nue",
    "Num photons-CC-numu", 
    "Num photons-NC"
  ...
  for(UInt_t idb=0; idb<nBranches; idb++){
      ...
      for (UInt_t idx=0; idx<nHistos; idx++){
          ...
          TCanvas *c1 = new TCanvas(Form("reco1_%d",idx),Form("%s_%s_ShwEff", s_branch_names.Data(), s_histo_names.Data()), 1000, 800);

         TString logname = plotname[idx]; 

          if (logname == "Hit Charge"){
            c1->cd();
            c1->SetLogy();
          }
	  ...
	  //histFile->Write();
          ...
          setStyle(hRef, 3, "entries");
          setStyle(hCur, 1, "entries");
          ...
          TString RefLegend="Pierre's production" /*+gRefVersion*/;
          TString CurLegend="fd_mc_he_2023a" /*+gCurVersion*/;
          ...
          topPad->cd();
          TPaveText *pt = new TPaveText(0.65, 0.65, 0.9, 0.9, "NDC");      // (0.35, 0.78, 0.75, 0.88, "NDC");
	        pt->AddText(plotname[idx]);
          pt->AddText(chisq);
          //pt->AddText(NDF);
          pt->SetFillStyle(0);
          pt->SetBorderSize(0);
          ...
          ratioPlotFile2->GetYaxis()->SetRangeUser(-3,3);
          ratioPlotFile2->Divide(hRef);
          setStyleRatio(ratioPlotFile2, gRefVersion.Data(), gCurVersion.Data(), Xaxisname[idx]);
